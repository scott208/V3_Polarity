...
def run(weights=ROOT / 'yolov3.pt',  # 模型路径
    ...
    # 将彩色图像转换为灰度
    if len(im.shape) == 3 and im.shape[2] == 3:  # 如果是RGB图像
        im = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)  # 转换为灰度
        im = np.expand_dims(im, axis=2)  # 添加通道维度
    
    im = torch.from_numpy(im).to(device)   # 5.2、处理每一张图片/视频的格式
    im = im.float()  # 强制使用float32
    im /= 255  # 归一化 0 - 255 to 0.0 - 1.0
    # 如果图片是3维(灰度) 就在前面添加一个维度1当中batch_size=1
    if len(im.shape) == 3:
        im = im[None]  # expand for batch dim [1, 1, H, W]
        ...
        # 后处理：将输出转换为uint8格式
        output = output.squeeze(0).cpu().numpy()  # [1, 1, H, W] -> [H, W]
                ...
                if dataset.mode == 'image':
                    ...
                    if vid_path[i] != save_path:
                        ...
                        if vid_cap:
                            ...
                            fps, w, h = 30, output.shape[1], output.shape[0]
                            save_path += '.mp4'
